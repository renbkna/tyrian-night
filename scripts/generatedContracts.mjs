...
export const DEFAULT_TYRIAN_THEME_LABEL = TYRIAN_THEME_CATALOG[0].label;
...
function buildBrokerContractTs() {
  const brokerPaths = ISLAND_BROKER_INSTALL_CONTRACT.brokerLibRoots.map((root) =>
    path.posix.join(root, ISLAND_BROKER_INSTALL_CONTRACT.brokerScriptName)
  );

  return `export const ISLAND_BROKER_INSTALL_CONTRACT = {
  brokerScriptName: ${formatTsString(ISLAND_BROKER_INSTALL_CONTRACT.brokerScriptName)},
  defaultAssetRoot: ${formatTsString(ISLAND_BROKER_INSTALL_CONTRACT.defaultAssetRoot)},
  defaultBrokerLibRoot: ${formatTsString(ISLAND_BROKER_INSTALL_CONTRACT.defaultBrokerLibRoot)},
  assetRoots: [
${ISLAND_BROKER_INSTALL_CONTRACT.assetRoots.map((root) => `    ${formatTsString(root)},`).join('\n')}
  ],
  brokerLibRoots: [${ISLAND_BROKER_INSTALL_CONTRACT.brokerLibRoots.map(formatTsString).join(', ')}],
  chownPath: ${formatTsString(ISLAND_BROKER_INSTALL_CONTRACT.chownPath)},
  nodePath: ${formatTsString(ISLAND_BROKER_INSTALL_CONTRACT.nodePath)},
  packageSlug: ${formatTsString(ISLAND_BROKER_INSTALL_CONTRACT.packageSlug)},
  pkexecPath: ${formatTsString(ISLAND_BROKER_INSTALL_CONTRACT.pkexecPath)},
} as const;
