...
Keep generated color values out of this template. Use `--tyrian-*` variables so hues continue to come from `source/themeFamilyContract.json`, per-variant lightness and chroma from `source/themes/`, aliases from `source/themeColorBindings.json`, and derived alpha from `source/themeOpacityContract.json`; role membership is owned by `source/themeRoleContract.json`.
